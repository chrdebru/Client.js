...
function crosses(a, b) {
  ...
  if (gA.type === 'LineString' && gB.type == 'LineString') {
    // test cross for L/L
    var featuredLine1 = turf.feature(gA);
    var featuredLine2 = turf.feature(gB);
    var crossPoint = turf.lineIntersect(featuredLine1,featuredLine2);
  ...
  if(gA.type === 'Point'){
    var featuredPoint = turf.feature(gA);
    switch (gB.type) {
    case 'LineString':
      var featuredLine = turf.feature(gB);
      //pointOnLine method can help find the nearest point from the given point on the line
      var nearestPoint = turf.pointOnLine(featuredPoint,featuredLine);
      if (nearestPoint !== undefined)
        return nearestPoint.properties.dist>0?XSD_FALSE:XSD_TRUE;
    case 'Polygon':
      var featuredGeom = turf.feature(gB);
...
function buffer(geom1, radius, units) {
  ...
  var buffer = turf.buffer(featuredGeom, N3Util.getLiteralValue(radius), N3Util.getLiteralValue(units));
  if (buffer !== undefined)
    return buffer;
}
