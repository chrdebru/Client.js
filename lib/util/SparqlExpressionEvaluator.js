...
var evaluators, operators,
    UnsupportedExpressionError, UnsupportedOperatorError, UnsupportedUnitError, InvalidArgumentsNumberError;
...
function buffer(geom1, radius, units) {
  ...
  var buffer = turf.buffer(featuredGeom, N3Util.getLiteralValue(radius), N3Util.getLiteralValue(parsedUnit));
  if (buffer !== undefined)
    return buffer;
}
