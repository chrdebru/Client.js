...
function distance(geom1, geom2, units){
  ...
  //TODO use a more elegant way to extend the geometry
  var point1 = {
    "type": "Feature",
    "geometry": gA
  };
  ...
  var distance = turf.distance(point1,point2,N3Util.getLiteralValue(units));
...
function buffer(geom1,radius,units){
  ...
  var featured_geom = turf.feature(geom);

  var buffer = turf.buffer(featured_geom,N3Util.getLiteralValue(radius),N3Util.getLiteralValue(units));
...
function convexHull(geom1){
  ...
  var convexHull = turf.convex(featured_geom);
...
function intersection(geom1,geom2){
  ...
  if(gA.type == "Polygon" && gB.type == "Polygon") {
    ...
    var featured_geom1 = turf.feature(gA);

    var featured_geom2 = turf.feature(gB);

    var intersection = turf.intersect(featured_geom1, featured_geom2);
...
function envelope(geom1){
  ...
  var envelope = turf.envelope(featured_geom);
...
function difference(geom1,geom2){
  ...
  if(gA.type == "Polygon" && gB.type == "Polygon") {
    ...
    var diff = turf.difference(featured_geom1, featured_geom2);
...
function symDifference(geom1,geom2){
  ...
  if(gA.type == "Polygon" && gB.type == "Polygon") {
    ...
    if (intersection != undefined) {
      ...
      var symDiff = turf.union(turf.difference(featured_geom1,intersection),turf.difference(featured_geom2,intersection))
...
['http://www.opengis.net/def/function/geosparql/union'].forEach(function (operatorName) {
